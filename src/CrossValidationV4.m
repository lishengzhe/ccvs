...
% cameraKu = {[0.5398    2.6854];[0.5397    1.4043];[0.2575 0.5846];[0.4377 0.5507];[0.4377 0.5507];[0 0]};
cameraKu = {[0.5398    2.6854];[0.5397    1.4043];[0 0];[ 0 0];[0 0];[0 0]};
...
abc = cell(length(subjectName), length(cameraName));
...
for j=1:length(cameraName)
    ...
    for i=1:length(subjectName)
        ...
        [ftc,resnorm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(hf2h,ftc1,[yf yh],h,[],[],options);
        ...
        c=ftc(3);
        h=h(1);
        abc{i,j}= [f*(c*t^2+h+c);t*f^2;t*h;f*(h*t^2+c*t^2+c)];
    end
end
